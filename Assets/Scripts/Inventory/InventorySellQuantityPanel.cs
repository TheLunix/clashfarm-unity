...
public class InventorySellQuantityPanel : MonoBehaviour
{
    [Header("Root")]
    public GameObject root;                 // корінь панелі (InventorySellQuantityPanel)
    ...
    /// <summary>
    /// Показати панель продажу.
    /// </summary>
    /// <param name="vm">ViewModel предмета</param>
    /// <param name="localizedName">Локалізована назва</param>
    /// <param name="pricePerOne">Ціна за 1 штуку (50% від BasePrice)</param>
    /// <param name="onConfirm">Колбек, який отримає кількість для продажу</param>
    /// <param name="onCancel">Колбек на "Ні"</param>
    public void Show(
        ...
        Action onCancel)
    {
        ...
        // Якщо стек >= 2 – показуємо CountRoot, інакше ховаємо (продаж 1 шт.)
        bool hasMultiple = _max > 1;
        if (countRoot != null)
            countRoot.SetActive(hasMultiple);

        _current = hasMultiple ? 1 : 1;
        ...
        if (icon != null)
        {
            ...
            {
                Debug.LogWarning("[InventorySellQuantityPanel] ItemIconProvider.Instance is null");
            }
        }
    }
    ...
    {
        // Число не може бути менше 1 (0 сенсу нема – є кнопка "Ні")
    ...
    private void UpdateDescLabel()
    {
        ...
        // Потім можна локалізувати, а поки – простий текст
        if (_max > 1)
        {
